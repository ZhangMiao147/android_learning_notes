...
　　为了能把一个 IP 报文放在不同的物理帧中，最大 IP 报文的长度就只能等于这条路径上所有物理网络的 MTU 的最小值。当数据报通过一个可以传输长度更大的帧的网络时，把数据报的大小限制在互联网上最小的 MTU 值是不经济；如果数据报的长度超过互联网上最小的 MTU 值的话，则当该数据报在穿越该子网时，就无法被封装在一个帧中。
