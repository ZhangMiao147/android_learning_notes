...
```
    <TextView
        android:id="@+id/activity_local_video_empty_video_tv"
        android:layout_width="match_parent"
        android:layout_height="match_parent"
        android:gravity="center"
        android:text="@string/no_local_video"
...
　　逻辑处理：
```
        if (localVideoBeanList == null || localVideoBeanList.size() < 1) {
         	//没有本地视频数据，显示提示
		 	tv_emptyVideo.setVisibility(View.VISIBLE);
        } else {
            //有本地视频数据，将数据显示在列表中
```
