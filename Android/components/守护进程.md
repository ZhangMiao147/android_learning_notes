...
　　缺点：前台服务和通知绑定在一起，意味着开启服务要伴随一条通知再通知栏，用户有感知。
...
　　缺点：触发时机不够实时，JobService 的触发时机回事充电时，闲暇时等特殊时机或者时周期性运行。
...
　　双进程守护的两个进程，一个进程用于应用所需的后台操作，且叫它本地进程，另一个进程只负责监听着本地进程的状态，再本地进程被杀的时候拉起，与此同时本地进程也在监听着这个进程，准备在它被杀时拉起，将这个进程称为远端进程。
