...
public class ACache {
 ...
 public static ACache getKx(Context ctx, String cacheName) {
  ...
  File f = new File(kxDir, cacheName); // 本质是文件存储
  return get(f, MAX_SIZE, MAX_COUNT);
 }
 ...
 public static ACache get(Context ctx, long max_zise, int max_count) {
  File f = new File(ctx.getCacheDir(), "ACache"); // 存储在 cache 目录下
 ...
 public void put(String key, String value) {
  File file = mCache.newFile(key); // 写入文件中
 ...
 public String getAsString(String key) {
  ...
  try {
   ...
   if (!Utils.isDue(readString)) { // 判断数据是否过期，未过期，则返回数据
  ...
  private void calculateCacheSizeAndCacheCount() {
   new Thread(new Runnable() {
    ...
    public void run() {
     int size = 0;
     int count = 0;
     File[] cachedFiles = cacheDir.listFiles(); // 文件列表
     if (cachedFiles != null) {
...
1. [Android 本地缓存ACache的简单使用和工具类](https://blog.csdn.net/Catherine6463/article/details/117333520)
