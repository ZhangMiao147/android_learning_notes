...
| 名称                                                         | 概要                                                | 详情                                                         |
| ------------------------------------------------------------ | --------------------------------------------------- | ------------------------------------------------------------ |
| [Retrofit2.0]()                                              | 以接口/注解的形式定义请求和响应                     | Square 开源的项目。是一套 RESTful 接口的 Android(Java) 客户端实现，基于注解，提供 JSON to POJO(Plain Ordinary Java Object，简单的 Java 对象)，POJO to JSON，网络请求（POST,GET,PUT,DELETE等）封装。本身的网络核心可以替换，如 Apache HTTP Client,URL connection,OKHTTP 等，数据解析核心也可以替换如 Gson,Jackson,fastjson,xStream等。力求用最少的代码，实现最强大的功能。 |
...
* 弱网情况下，在连接失败后，OkHttp 会自动进行重试，特别是有备用地址时还会通过备用地址进行连接。而安全上，其支持新一代的 TLS 功能（HTTPS 的加密功能）、SNI（Server Name Indication，指定了 TLS 握手时要连接的主机名，SNI 协议是为了支持同一个 IP（和端口）支持多个域名） 和 ALPN（Application-Layer Protocol Negptiation，也是 TLS 层的扩展，用于协商应用层使用的协议），如果服务器不支持的化则会自动降级到 TLS 1.0。
...
　　AsyncCall 处理网络请求任务是通过责任链模式来完成的，会先去初始化责任链，责任链初始化完成后，就执行责任链，链的就是拦截器。
...
## 3.4. okhttp源码：分发器和五大拦截器； 

　　Okhttp 的同步请求和以部请求都会交给分发器去执行，同步请求时将任务直接加入到同步执行队列中，异步请求是将任务异步运行队列中，并且会交给线程池执行。

　　而任务的执行主要靠拦截器来完成。

## 3.5. 你们用的 okhttp ？那你有没有做过一些网络优化呢？比如弱网环境。

目的：主要是减少用户等待时间，提升用户体验

可以优化的地方：

1. 前后端采用 gzip 方式请求和响应

前端在请求header添加："content-encoding" 为 "gzip"

后端也要开启 gzip，才能生效

相比不采用 gzip 的请求方式，能节省流量，可以快速响应

android：OKHttp 请求框架默认支持 gzip，不需要额外配置

2. 图片

 （1）不同分辨率的图片：先用图片控件宽高占位显示，根据网络情况 3g 4g wifi 请求不同分辨率图片的方式

 （2）用小的图片格式：比如 webp，一张图片 jpg 的图片转换成 webp 格式，大小甚至达到十分之一。

3. 数据库

  用数据库缓存配合网络请求完成数据的显示，

   例子：先读取数据库中数据显示，网络请求并行进行，响应后更新 UI 显示和数据库数据。
...
　　Retrofit 的主要原理是利用了 Java 的动态代理技术，把 ApiService 的方法调用集中到了 InvocationHandler.invoke，构建了 ServiceMethod 、OkHttpClient，返回 callAdapter.adapter 的结果。
...
```java
// Proxy.newProxy() 的 InvocationHandler 的 invoke() 方法
...
## 5.1. Glide 与其他图片开源库的比较

　　老牌图片加载框架 UniversalImageLoader、 Google 推出的 Volley，后来的 Glide 和 Picasso，还有 Facebook 的 Fresco。每一个都非常稳定，功能也都十分强大。但是它们的使用场景基本都是重合的。

　　从易用性上来讲，Glide 和 Picasso 应该都是完胜其他框架的，这两个框架都实在是太简单好用了，大多数情况下加载图片都是一行代码就能解决的，而 UniversalImageLoader 和 Fresco 则在这方面略逊一些。

　　Glide 和 Picasso 这两个框架的用法非常相似，但其实它们各有特色。Picasso 比 Glide 更加简洁和轻量，Glide 比 Picasso 功能更为丰富。Glide 支持播放 GIF ，Picasso 不支持。

　　Glide 和 Picasso 的用法是非常相似的，但是有一点差别很大。Glide 加载图片的默认格式是 RGB_565，而 Picasso 加载图片的默认格式是 ARGB_8888。ARGB_8888 格式的图片效果会更加细腻，但是内存开销会比较大，而 RGB_565 格式的图片则更加节省内存，但是图片效果上会差一些。Glide 也可以通过设置使用 ARGB_888 的数据格式。
...
1. with() 方法就是为了得到一个 RequestManager 对象而已，然后 Glide 会根据传入的 with() 方法的参数来确定图片加载的生命周期。

   如果是 Application 参数，调用  getApplicationManager() 方法来获取一个 RequestManager 对象。Application 对象的生命周期即应用程序的生命周期。

   如果是非 Application 参数，调用 getSupportRequestManagerFragment 方法向当前的 Activity 当中添加一个隐藏的 Fragment（为了 Glide 直到加载的生命周期）。
...
3. into() 方法主要分为三个部分：1. 请求数据；2. 将请求的数据解析转换为图片格式；3. 将图片显示到 ImageView 上。into() 方法会创建 Target 对象（负责将占位符，错误占位图，图片显示在 ImageView 上，就是控制 view 的显示）和创建 GenericRequest 对象（负责请求和请求回调），1. 请求数据：GenericRequest 会创建 EngineJob 对象获取数据，EngineJob 对象会在线程池中运行获取数据的操作，获取数据的时候会先从缓存中获取，如果缓存中没有，则从资源中获取，在资源中获取数据是调用的 DataFetcher 的 loadData() 方法。2.解析数据：获得数据后调用 ResourceDecoder 类的 decode() 方法解析数据，并且会对图片进行压缩、旋转、圆角等处理，数据转换成了 Bitmap 或者 Gif 数据格式，最后会将 Bitmap 和 Gif 数据封装到 GifBitmapWrapper 中，会将获取到的 Resource 数据进行缓存。3.显示图片：将 GifBitmapWrapper 转换为 Bitmap 或者 Gif 可以直接使用的数据后，会采用 handler 向主线程发送 Message 消息将结果传给主线程，在消息处理中会调用到 ImageViewTarget 的 setResource() 方法将图片显示到控件上。

4. Glide 缓存机制

   Glide 的缓存分成了两个模块，一个是内存缓存，一个是硬件缓存。内存缓存的主要作用是防止应用重复将图片数据读取到内存当中，而硬件缓存的主要作用是防止应用重复从网络或其他地方重读下载和读取数据。

   缓存的 Key 是 EngineKey ，由图片的地址、大小等因素共同构成。
   ...
   * 内存缓存
     ...
     在 load() 方法过程中会先去获取内存缓存图片，如果有缓存，会将缓存从内存缓冲中取出，放入一个 HashMap 中表示正在显示的数据，然后则直接将图片显示在控件上，如果不存在缓存，则调用网络请求，在请求处理完成后，切换到主线程中，会将数据从 HashMap 中移除，存储到内存缓存中。

   * 硬盘缓存

     硬盘缓存的实现也是使用的是 DiskLruCache，内存缓存是在调用 load() 方法开始就会去检查内存缓存，没有内存缓存就会进入线程，开始运行线程时才会去检查硬盘缓存。硬盘的缓存也是在请求数据完成后在线程中存储的。
...
　　**解决方法：**自定义一个 MyGlideUrl 类继承 GlideUrl，重写 getCacheKey() 方法，在这个方法中对图片的 url 进行处理。使用是 `.load(new MyGlideUrl(url))`。

### 5.4.2. 阿里 OSS 的图片显示问题

　　使用阿里的 oss 存储图片，在后面显示 oss 的地址图片无法显示，会报错没有不匹配的 Model。

　　**解决方法**：自定义适配器。

https://blog.csdn.net/qq_35651451/article/details/88665127

## 5.5. Glide是如何加载Gif图片的

[原来Glide是这样加载GIF图的](https://blog.csdn.net/Coo123_/article/details/106428226)

一帧一帧的获取，向主线程发消息刷新界面，绘制当前帧的图像。
