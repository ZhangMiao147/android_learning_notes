...
2. load() 方法会根据传递的参数确定使用的 streamModelLoader 和 fileDescriptorModelLoader，创建一个 ImageVideoModelLoader 对象，streamModelLoader 和 fileDescriptorModelLoader 作为它的成员变量，DrawableTypeRequest 会创建一个 FixedLoadProvider 对象，FixedLoadProvider 持有 ImageVideoModelLoader 对象，而 DrawableTypeRequest 持有 FixedLoadProvider 对象。load() 方法返回的是 DrawableTypeRequest 对象。

3. into() 方法会创建 Target 对象（负责将占位符，错误占位图，图片显示在 ImageView 上）和创建 GenericRequest 对象，1. 请求数据：GenericRequest 会创建 EngineJob 对向获取数据，EngineJob 对象会在线程持中运行获取数据的操作，获取数据的时候会先从缓存中获取，如果缓存中没有，则从资源中获取，在资源中获取数据是调用的 DataFetcher 的 loadData() 方法。2.解析数据：获得数据后调用 ResourceDecoder 类的 decode() 方法解析数据，并且会对图片进行压缩、旋转、圆角等处理，数据转换成了 Bitmap 或者 Gif 数据格式，最后会将 Bitmap 和 Gif 数据封装到 GifBitmapWrapper 中，会将获取到的 Resource 数据进行缓存。3.显示图片：将 GifBitmaoWrapper 转换为 Bitmap 或者 Gif 可以直接使用的数据后，会采用 handler 向主线程发送 Message 消息将结果传给主线程，在消息处理中会调用到 ImageViewTarget 的 setResource() 方法将图片显示到控件上。

4. with() 方法如果传入的不是 Application 的 Context，就会向当前的 Context 添加一个隐藏的 fragment，这主要是为了知道加载的生命周期，而 with() 会返回一个 RequestManager 对象。

5. load() 方法会创建 Glide 的实例，并调用了 Glide 的初始化，对缓存、下载等对象进行初始化，注册好 ModelLoader 的工厂模式，创建 DrawableTypeRequest 对象并返回，DrawableTypeRequest 包含 streamModelLoader 和 fileDescriptorModelLoader 两个 ModelLoader。
