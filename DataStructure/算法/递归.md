...
　　当 n=1 的时候说明已经到了最小解了，因为 n! 依赖 (n-1)! 的结果，后面以此类推，最后到了 1 的阶乘，前面的以来就依次有解了。
...
2. 操作数栈（Operand Stack）
   ...
   比如在方法中执行 a+b，首先会把 a 和 b 压入操作数栈中，然后把 a 和 b 弹出栈进行相家，然后把结果压入操作数栈中。

   又比如在方法中调用另一个方法，操作数栈用于保护被调用方法的参数和接受该方法的返回值。
...
　　Java 没有对尾递归做任何优化处理，Java 虽然没有对尾部递归做出优化，但是尾部递归式没有 “ 回归阶段 ”，因为最后一次方法的执行就可以得到最终结果，可以在 Eclipse 上分别对上面的两种递归调试下。
