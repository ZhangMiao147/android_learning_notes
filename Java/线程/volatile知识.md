...
　　安全使用 volatile 的另一种简单模式是定期发布观察结果供成簇内部使用。
...
　　如果更新不频繁的话，该方法可实现更好的性能，因为读路径的开销仅仅设计 volatile 读操作，这通常要优于一个无竞争的锁获取的开销。
...
　　也推荐懒加载优雅写法 Initialization on Demand Holder(IODH)：
