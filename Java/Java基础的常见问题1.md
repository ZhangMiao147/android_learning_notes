...
1. 浮点型，float 数据类型是单精度、32 位、符合 IEEE 754 标准的浮点数，占 4 字节，浮点数是由舍入误差的。
2. 数据范围在 (-2^128)~(-2^(-23-126))-(0)-(2^-149)~2^128，直接赋值时必须在数据后加上 f 或者 F。
...
2. 数据范围在 -2^1024~(-2^(-1022-52))-0-(2^－1074)~2^1024，赋值时可以加上 d 或者 D，也可以不加。
...
　　“引用“ 是存储在有序的内存栈上的，而对象本身的值存储在内存对上的。
